 ...
 *   - User context (frontend calls): Uses user-scoped auth
 *   - Service-role context (backend-to-backend calls): Uses service-role auth
 *
 * Called by:
 *   - ProjectForm.jsx (frontend, on product/package save)
 *   - ProjectProductsPackages.jsx (frontend, on batch product update)
 *   - recalculateProjectPricingServerSide.ts (backend, delegates all math here)
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // Dual-context auth: accept user auth (frontend) OR service-role (backend-to-backend)
    if (!user) {
      try {
        // Probe: if asServiceRole works, this is a legitimate backend call
    ...
    // Entity accessor: user-context for frontend calls, service-role for backend calls
    const db = user ? base44.entities : base44.asServiceRole.entities;

    // Rate limit: only applies to user calls (backend calls are already throttled by their callers)
    if (user) {
      const now = Date.now();
    ...
    if (body?._health_check) {
      return Response.json({ _version: 'v2.0', _fn: 'calculateProjectPricing', _ts: '2026-03-17' });
    }
    ...
    if (!Array.isArray(products) || !Array.isArray(packages)) {
      return Response.json({ error: 'products and packages must be arrays' }, { status: 400 });
    }
    ...
    const [agentMatrix, agencyMatrix, allProductsRaw, allPackagesRaw] = await Promise.all([
      agent_id ? retryWithBackoff(() => db.PriceMatrix.filter({ entity_type: 'agent', entity_id: agent_id })) : Promise.resolve([]),
      agency_id ? retryWithBackoff(() => db.PriceMatrix.filter({ entity_type: 'agency', entity_id: agency_id })) : Promise.resolve([]),
      Promise.all(
        [...new Set([
          // Standalone product IDs
          ...products.map((p: any) => p.product_id),
          // Nested product IDs inside packages (critical: needed for extra qty calculation)
    ...
    // Fix O: Pick best matrix — prefer project-type-scoped, fall back to unscoped (global)
    ...
    // Helper: apply matrix override or use master default
    ...
    for (const item of products) {
      ...
      // Check matrix overrides for base AND unit price
    ...
    // Packages (with nested products — extra qty above included amount is charged)
    for (const pkg of packages) {
      ...
      // Calculate extra costs for nested products above included quantities
      ...
      for (const masterProd of masterProducts) {
        ...
        // Only per_unit products charge for extras
        if (product.pricing_type === 'per_unit') {
          ...
          if (extraQty > 0) {
            ...
            // Fix K: Apply matrix overrides to nested product unit price (same as standalone)
    ...
    // Fix J: Apply blanket discount — product_percent to products, package_percent to packages
    ...
    // Return result (NO final rounding — price is already $5-aligned from line items)
    return Response.json({
      success: true,
