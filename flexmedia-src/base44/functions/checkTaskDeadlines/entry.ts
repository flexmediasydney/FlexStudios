...
async function _resolveUserIds(base44, roles, projectId) {
...
async function _checkNotifPref(base44, userId, type, category) {
  try {
    const prefs = await base44.asServiceRole.entities.NotificationPreference.list('-created_date', 500);
    const up = prefs.filter((p) => p.user_id === userId);
    const tp = up.find((p) => p.notification_type === type);
    if (tp !== undefined) return tp.in_app_enabled !== false;
    const cp = up.find((p) => p.category === category && (!p.notification_type || p.notification_type === '*'));
    if (cp !== undefined) return cp.in_app_enabled !== false;
    return true;
  } catch { return true; }
}

async function _isDupNotif(base44, key, userId) {
  try {
    const recent = await base44.asServiceRole.entities.Notification.list('-created_date', 500);
    return recent.some((n) => n.idempotency_key === key && n.user_id === userId);
  } catch { return false; }
}

async function _createNotif(base44, p) {
  const allowed = await _checkNotifPref(base44, p.userId, p.type, p.category);
  if (!allowed) return false;
  if (p.idempotencyKey && await _isDupNotif(base44, p.idempotencyKey, p.userId)) return false;
...
Deno.serve(async (_req) => {
  ...
  try {
    ...
    const tasks = await base44.asServiceRole.entities.ProjectTask.list('-created_date', 2000);
    ...
    for (const task of activeTasks) {
      ...
      if (hoursUntilDue < 0) {
        ...
        const ok = await _createNotif(base44, {
    ...
    try {
      const timeLogs = await base44.asServiceRole.entities.TaskTimeLog.list('-created_date', 1000);
      const runningLong = timeLogs.filter((t) =>
        t.start_time &&
        !t.end_time &&
        t.status === 'running' &&
    ...
    try {
      ...
      for (const project of allProjects) {
        ...
        if (
          ...
          if (overdueDays >= 1) {
            const adminIds = await _resolveUserIds(base44, ['master_admin'], null);
    ...
    try {
      const deliveredProjects = await base44.asServiceRole.entities.Project.filter(
        { status: 'delivered' }, null, 500
      );
