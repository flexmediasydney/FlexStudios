...
Deno.serve(async (req) => {
  try {
    ...
    if (body?._health_check) {
      return Response.json({ _version: 'v2.0', _fn: 'healthCheckFunctions', _ts: '2026-03-17' });
    }
    ...
    for (const [category, functions] of Object.entries(CRITICAL_FUNCTIONS)) {
      for (const fnName of functions) {
        ...
        try {
          const response = await base44.functions.invoke(fnName, { _health_check: true });
    ...
    // Smoke tests — actually exercise critical paths
    ...
    if (include_smoke_tests) {
      ...
      try {
        ...
        const res = await base44.functions.invoke('calculateProjectTaskDeadlines', {
      ...
      try {
        ...
        const res = await base44.functions.invoke('receiveTonomoWebhook', {
          orderId: 'test_smoke_check', action: 'test',
        });
        const t3Data = res?.data || res;
        smokeTests.push({
    ...
    if (include_data_integrity) {
      try {
        const [projects, tasks, products, packages, timeLogs, efforts] = await Promise.all([
          base44.entities.Project.list(null, 2000),
          base44.entities.ProjectTask.filter({}, null, 5000),
          base44.entities.Product.list(null, 500),
          base44.entities.Package.list(null, 200),
          base44.entities.TaskTimeLog.filter({}, null, 2000),
          base44.entities.ProjectEffort.filter({}, null, 500),
        ]);
        ...
        const lastWebhookLog = await base44.entities.TonomoWebhookLog.list('-received_at', 1).catch(() => []);
        const lastQueueItem = await base44.entities.TonomoProcessingQueue.list('-created_at', 1).catch(() => []);
      ...
      try {
        ...
        for (const admin of admins) {
          await base44.asServiceRole.entities.Notification.create({
            user_id: admin.id, type: 'system_alert', category: 'system', severity: 'critical',
            title: `⚠️ ${dead + stale} backend function issue(s)!`,
