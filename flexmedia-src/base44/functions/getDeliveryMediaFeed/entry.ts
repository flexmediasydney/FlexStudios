...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!token) return Response.json({ error: 'DROPBOX_API_TOKEN not configured' }, { status: 500 });
    ...
    if (!path && !share_url) return Response.json({ error: 'path or share_url required' }, { status: 400 });
    ...
    const files = allEntries
      .filter(e => e['.tag'] === 'file')
      .filter(e => !SKIP_EXTS.has((e.name.split('.').pop() || '').toLowerCase()))
      .sort((a, b) => new Date(b.client_modified).getTime() - new Date(a.client_modified).getTime())
    ...
    const nonThumb      = files.filter(f => !canThumbnail(f.name));
    ...
    const result = files.map(f => ({
      ...
      // Video duration from Dropbox media_info if available
