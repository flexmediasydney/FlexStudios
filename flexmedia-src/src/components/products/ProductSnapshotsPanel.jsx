...
import { Camera, Plus, ChevronDown, ChevronRight, Download } from "lucide-react";
...
export default function ProductSnapshotsPanel() {
  const [expanded, setExpanded] = useState({});
  const [creating, setCreating] = useState(false);
  const { data: snapshots, loading } = useEntityList("ProductSnapshot", "-created_date");
  ...
  const handleManualSnapshot = async () => {
    ...
    try {
      ...
      const products = await api.entities.Product.list("name", 1000);
      const now = new Date();
      const label = format(now, "MMMM yyyy") + " (Manual)";

      await api.entities.ProductSnapshot.create({
  ...
  const handleExport = (snapshot) => {
    ...
    a.href = url;
    a.download = `products-snapshot-${snapshot.snapshot_label}.json`;
    a.click();
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Point-in-time captures of all product pricing. Use these to audit or restore previous configurations.
        </p>
      ...
      {snapshots.length === 0 ? (
        <Card className="p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">Take your first snapshot to capture the current product pricing state.</p>
        </Card>
      ) : (
                    ...
                    <tbody>
                      {snap.data.map((p, i) => (
                        <tr key={i} className={i < snap.data.length - 1 ? "border-b" : ""}>
                          ...
                          <td className="px-3 py-2">${p.standard_tier?.base_price?.toFixed(2) || "0.00"}</td>
                          <td className="px-3 py-2">${p.premium_tier?.base_price?.toFixed(2) || "0.00"}</td>
