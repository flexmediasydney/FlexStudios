...
## ⚡ Execution shape change (2026-04-29) — read this first

**This spec was authored assuming the two-pass architecture (Pass 1 + Pass 2 separate).** Joseph's 2026-04-29 review concluded that with Opus 4.7 + this universal schema's compact per-image shape, the right execution pattern is a **single unified call** (Pass 1 + Pass 2 collapsed). See `W11-7-unified-shortlisting-architecture.md`.

What this means for W11:

- **The schema itself is unchanged.** Every field in the `UniversalVisionResponse` interface below remains correct.
- **The producer changes.** Instead of Pass 1 emitting the schema per-image and Pass 2 consuming it as text, the unified call's `compositions[]` array IS the universal schema for every image AND its `slot_decisions[]` array IS the cross-image selection output.
- **Per-image rich descriptions move to async backfill.** Today's spec assumes Pass 1 emits the 150-word `analysis` paragraph synchronously. Under W11.7, this paragraph is generated by an async Sonnet 4.6 backfill job after the unified call returns. The schema's `analysis` field remains; only the production timing changes.
- **W11 ships first; W11.7 ships after.** The unified architecture depends on W11's compact schema fitting in an 8k output token budget per Opus call.
