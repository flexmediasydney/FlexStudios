...
| [W11-7-unified-shortlisting-architecture.md](./W11-7-unified-shortlisting-architecture.md) | W11.7 | 🛑 keystone | **Architectural keystone** — Shape D 5-call multi-stage Gemini-anchored architecture. 3-4 Stage 1 batch calls × 50 images each (full per-image enrichment: analysis, scoring, classification, clutter, per-image listing copy, appeal/concern_signals, retouch_priority, gallery_position_hint, style_archetype) + 1 Stage 4 visual master synthesis call across all 200 images that emits slot decisions + master_listing block (voice_tier, canonical_id refs, embedding_anchor_text). Primary vendor: Gemini 2.5 Pro per Saladine A/B iter-4. Anthropic Opus 4.7 retained as failover. ~$3.84 per 200-angle shoot. 4-phase rollout (coexistence → pilot → default → deprecate). Mig 349 |
| [W11-7-7-master-listing-copy-synthesis.md](./W11-7-7-master-listing-copy-synthesis.md) | W11.7.7 | ⚙️ being authored | Stage 4 master_listing copy output: structure, fields, validators, voice/grade-level constraints, lifecycle in `master_listings` table, integration with delivery |
| [W11-7-8-voice-tier-modulation.md](./W11-7-8-voice-tier-modulation.md) | W11.7.8 | ⚙️ being authored | Premium / Standard / Approachable tier system: package mapping, modulation rules at Stage 4, override paths, audit signals |
| [W11-7-9-master-class-prompt-enrichment.md](./W11-7-9-master-class-prompt-enrichment.md) | W11.7.9 | ⚙️ being authored | Perceptual rubric, Sydney market primer, voice exemplars, 5-level canonical vocabulary hierarchy that anchor both Stage 1 and Stage 4 prompts |
