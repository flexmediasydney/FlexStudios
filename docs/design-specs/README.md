...
| [W11-7-unified-shortlisting-architecture.md](./W11-7-unified-shortlisting-architecture.md) | W11.7 | 🛑 keystone | **Architectural keystone** — collapse Pass 1 + Pass 2 into single Opus 4.7 call + async Sonnet description backfill. Multi-message + prompt-caching for >50-group rounds. Project memory + canonical registry as prompt context = closed-loop "engine grows per project". 4-phase rollout (coexistence → pilot → default → deprecate). Mig 349 |
| [W11-8-multi-vendor-vision-adapter.md](./W11-8-multi-vendor-vision-adapter.md) | W11.8 | ⚙️ ready | Vendor abstraction over Anthropic / Google / OpenAI vision APIs. Per-pass model config in `engine_settings`. Shadow-run A/B harness — first test target: 13 Saladine round (`3ed54b53`, 42 compositions) compared across 5 vendor configs. Defensive insurance against vendor lock-in / pricing / outage. Mig 350 |
