  ...
  try {
    ...
    if (!user || user.role !== "master_admin") {
      return Response.json(
        { error: "Only master_admin can run tests" },
        { status: 403 }
      );
    }
    ...
    try {
      const existingPerms = await base44.asServiceRole.entities.Permission.list();
    ...
    try {
      const perms = await base44.asServiceRole.entities.Permission.list();
      const hasRequiredFields = perms.every(p =>
    ...
    try {
      const perms = await base44.asServiceRole.entities.Permission.filter({ name: "projects.view" });
      ...
      if (perms.length > 0 && users.length > 0) {
        ...
        if (existing.length === 0) {
          const grant = await base44.asServiceRole.entities.UserPermission.create({
            user_id: testUser.id,
          ...
          // Clean up
          await base44.asServiceRole.entities.UserPermission.delete(grant.id);
        } else {
          results.push({
            test: "UserPermission Creation",
            status: "PASS",
    ...
    try {
      const log = await base44.asServiceRole.entities.PermissionAuditLog.create({
      ...
      // Clean up
      await base44.asServiceRole.entities.PermissionAuditLog.delete(log.id);
    } catch (e) {
      results.push({
        test: "PermissionAuditLog Creation",
        status: "FAIL",
    ...
    try {
      const perms = await base44.asServiceRole.entities.Permission.list();
      const hierarchyValid = perms.every(p => {
        if (!p.parent_permission_id) return true;
        return perms.some(parent => parent.id === p.parent_permission_id);
      });
      results.push({
    ...
    try {
      ...
      const scopeValid = perms.every(p =>
