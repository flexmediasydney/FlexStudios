...
Deno.serve(async (req) => {
  try {
    ...
    const uniqueLabelsToRemove = Array.from(new Set(labels.map(l => l.trim())));
    ...
    for (const message of messages) {
      try {
        ...
        const newLabels = currentLabels.filter(l => !uniqueLabelsToRemove.includes(l));
    ...
    return Response.json({ 
      error: 'Failed to remove labels: ' + (error?.message || 'Unknown error') 
    }, { status: 500 });
  }
});