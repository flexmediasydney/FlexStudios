...
// IDEMPOTENCY CHECK
// ═══════════════════════════════════════════════════════════════════════════
async function isDuplicate(base44: any, idempotencyKey: string, userId: string): Promise<boolean> {
  try {
    // Filter by user_id first so the window covers 500 notifications per user,
    // not 500 across all users
...
async function createNotificationForUser(
  ...
  const allowed = await checkPreference(base44, params.userId, params.type, category);
  if (!allowed) return { skipped: true, reason: 'preference_disabled' };

  if (params.idempotencyKey) {
    const dup = await isDuplicate(base44, params.idempotencyKey, params.userId);
...
async function createNotificationsForRoles(
  ...
  const userIds = await resolveUserIds(base44, params.roles, params.projectId);
  let created = 0;
  let skipped = 0;
  ...
  for (const userId of userIds) {
    ...
    const result = await createNotificationForUser(base44, {
      ...params,
      userId,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create') {
      const result = await createNotificationForUser(base44, params);
      return Response.json(result);
    }

    if (action === 'create_for_roles') {
      const result = await createNotificationsForRoles(base44, params);
      return Response.json(result);
    }

    if (action === 'get_type_registry') {
      return Response.json({ types: NOTIFICATION_TYPES });
    }
