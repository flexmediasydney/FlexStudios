...
Deno.serve(async (req) => {
  try {
    ...
    try {
      ...
      await base44.asServiceRole.entities.EmailAccount.update(account.id, { access_token: accessToken });

      // Build query based on last sync
      let query = '';
      if (account.last_sync) {
        const lastSyncDate = new Date(account.last_sync);
        // Go back 5 minutes to catch any missed emails
      ...
      for (let i = 0; i < messageIds.length; i += BATCH_SIZE) {
        ...
        for (const msg of batch) {
          try {
            // Check if message exists (with limit to prevent over-fetching)
            ...
            if (existing.length === 0) {
              ...
              let threadProjectLink = null;
              try {
                const siblings = await base44.asServiceRole.entities.EmailMessage.filter({
                  email_account_id: account.id,
              ...
              if (msgResponse.ok) {
                ...
                const savedMessage = await base44.asServiceRole.entities.EmailMessage.create({
                  email_account_id: account.id,
                  ...
                  try {
                    ...
                    if (threadProjectLink?.project_id) {
                      const project = await base44.asServiceRole.entities.Project.get(threadProjectLink.project_id).catch(() => null);
