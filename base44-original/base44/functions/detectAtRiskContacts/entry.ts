...
Deno.serve(async (req) => {
  ...
  try {
    const agents = await db.Agent.list('-created_date', 1000);
    const watchedAgents = agents.filter((a: any) =>
      a.relationship_state === 'Active' || a.relationship_state === 'Prospecting'
    );

    const projects = await db.Project.list('-shoot_date', 2000);
    const now = Date.now();
    ...
    for (const agent of watchedAgents) {
      ...
      if (shouldBeAtRisk !== (agent.is_at_risk === true)) {
        await db.Agent.update(agent.id, { is_at_risk: shouldBeAtRisk }).catch(() => {});
        if (shouldBeAtRisk) {
    ...
    if (newlyAtRisk.length > 0) {
      const adminUsers = await db.User.list('-created_date', 200);
      ...
      for (const admin of admins) {
        const allowed = await _canNotify(base44, admin.id, 'stale_project', 'system');
        if (!allowed) continue;
