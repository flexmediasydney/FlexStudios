...
//   - functions/calculateProjectTaskDeadlines.ts  (this file)
//   - functions/calculateTaskBlockingState.ts
...
function areDependenciesComplete(task, allTasks) {
  ...
  return task.depends_on_task_ids.every(depId => {
    const dep = allTasks.find(t => t.id === depId);
    // If the dependency no longer exists (hard-deleted) or is soft-deleted,
    // treat it as satisfied — don't block the dependent task forever
...
Deno.serve(async (req) => {
  try {
    ...
    // Accept both user sessions and service-role calls (from trackProjectStageChange,
    // syncProjectTasksFromProducts, calculateTaskBlockingState)
    const db = user ? base44.entities : base44.asServiceRole.entities;

    const body = await req.json().catch(() => ({}));

    if (body?._health_check) {
      return Response.json({ _version: 'v2.1', _fn: 'calculateProjectTaskDeadlines', _ts: '2026-03-17' });
    }
    ...
    try {
      project = await db.Project.get(project_id);
      allTasks = await db.ProjectTask.filter({ project_id });
    } catch (err) {
      if (err.status === 404) return Response.json({ error: 'Project not found' }, { status: 404 });
      // Silently handle rate limit—return partial results
      if (err.status === 429) {
        return Response.json({ success: true, updated_count: 0, total_tasks: 0, note: 'Rate limited' });
      }
      throw err;
    }
    ...
    for (const task of allTasks) {
      ...
      // Honour manual due date overrides — skip auto-calculation for these tasks
    ...
    if (dry_run) {
      return Response.json({ success: true, dry_run: true, would_update_count: stateChanges.length, changes: stateChanges, debug: debugInfo });
    }
    ...
    // Batch updates — 5 at a time to avoid rate limiting
    const batchSize = 5;
    for (let i = 0; i < stateChanges.length; i += batchSize) {
      ...
      await Promise.all(batch.map(async change => {
        try {
          await retryWithBackoff(() => db.ProjectTask.update(change.id, change.data));
          results.success++;
        } catch (err) {
          results.failed++;
