...
serveWithAudit('removeLabelsFromEmails', async (req) => {
  ...
  try {
    ...
    const uniqueLabelsToRemove = Array.from(new Set(labels.map((l: string) => l.trim())));
    ...
    for (const message of messages) {
      try {
        ...
        const newLabels = currentLabels.filter((l: string) => !uniqueLabelsToRemove.includes(l));
    ...
    const response: any = {
      ...
      message: `Removed labels from ${updated}/${messages.length} emails`,
    };
    ...
    return errorResponse('Failed to remove labels: ' + (err?.message || 'Unknown error'));
  }
});
