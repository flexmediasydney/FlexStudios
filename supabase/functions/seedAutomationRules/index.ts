...
serveWithAudit('seedAutomationRules', async (req) => {
  ...
  try {
    ...
    const existing = await entities.ProjectAutomationRule.list('-created_date', 5);
    if (existing?.length > 0) {
      return jsonResponse({ message: 'Already seeded, skipping', count: existing.length });
    }
    ...
    const rules = [
      ...
      },
      {
        ...
        action_config: JSON.stringify({ message: 'Overdue: shoot date has passed but project is still in Scheduled stage. Please review.' }),
        cooldown_minutes: 1440,
        dry_run_only: false,
      ...
      },
      {
        ...
        description: 'If a project in uploaded or later receives a reschedule signal (shoot_date changes), notify the owner.',
        rule_group: 'scheduling',
        is_enabled: true,
        is_system: true,
        priority: 5,
      ...
      // PRODUCTION GROUP
      {
        ...
        condition_logic: 'AND',
        action_type: 'add_activity_log',
        action_config: JSON.stringify({ message: 'Project has been in Stills in Progress for 3+ days with no update. Please review production status.' }),
        cooldown_minutes: 1440,
        dry_run_only: false,
      ...
      },
      {
        ...
        condition_logic: 'AND',
        action_type: 'add_activity_log',
        action_config: JSON.stringify({ message: 'Service assignment is uncertain for this booking (multiple photographers with ambiguous service split). Please verify task assignments manually.' }),
        cooldown_minutes: 1440,
        dry_run_only: false,
      ...
      },
      {
        ...
        condition_logic: 'AND',
        action_type: 'add_activity_log',
        action_config: JSON.stringify({ message: 'Project approved from pending_review but mapping gaps remain. Some Tonomo data may not have mapped correctly — check the Tonomo tab.' }),
        cooldown_minutes: 60,
        dry_run_only: false,
      ...
      // QUALITY GROUP
      {
        ...
        condition_logic: 'AND',
        action_type: 'add_activity_log',
        action_config: JSON.stringify({ message: 'Project has had no updates in 7+ days. Is it stuck?' }),
        cooldown_minutes: 1440,
        dry_run_only: false,
      ...
      },
      {
        ...
        description: 'Daily at 8am: write a heartbeat entry to confirm automation is running.',
        rule_group: 'quality',
        is_enabled: false,
        is_system: false,
        priority: 100,
    ...
    for (const rule of rules) {
      try {
        await entities.ProjectAutomationRule.create({
          ...rule,
          fire_count: 0,
          skip_count: 0,
    ...
    return jsonResponse({ message: `Seeded ${created} of ${rules.length} rules`, created });
  } catch (error: any) {
