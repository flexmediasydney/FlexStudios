...
serveWithAudit('deepLogicAuditProjectTypes', async (req) => {
  ...
  try {
    ...
    const projectTypeMap = new Map(projectTypes.map((pt: any) => [pt.id, pt]));
    ...
    for (const pkg of packages) {
      ...
      if (pkg.products && Array.isArray(pkg.products)) {
        for (const nestedProd of pkg.products) {
          ...
          const prodTypes = prod.project_type_ids || [];
          if (prodTypes.length > 0 && !prodTypes.some((t: string) => pkg.project_type_ids.includes(t))) {
            findings.critical.push({
    ...
    for (const proj of projects) {
      const projTasks = projectTasks.filter((t: any) => t.project_id === proj.id && !t.is_deleted);
      const projEffort = projectEfforts.find((e: any) => e.project_id === proj.id);
    ...
    for (const matrix of priceMatrices) {
      if (matrix.blanket_discount?.enabled && !matrix.use_default_pricing) {
        const hasProductOverrides = matrix.product_pricing?.some((p: any) => p.override_enabled);
        const hasPackageOverrides = matrix.package_pricing?.some((p: any) => p.override_enabled);
    ...
    for (const prod of products) {
      const checkTemplates = (templates: any[], tier: string) => {
        if (!templates) return;
    ...
    for (const pkg of packages) {
      ...
      const prodTypes = pkg.products.map((p: any) => productMap.get(p.product_id)).filter(Boolean);
      const hasFixedAndPerUnit = prodTypes.some((p: any) => p.pricing_type === 'fixed') &&
                                  prodTypes.some((p: any) => p.pricing_type === 'per_unit');
