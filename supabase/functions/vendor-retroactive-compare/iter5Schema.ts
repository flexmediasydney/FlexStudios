/**
 * iter5Schema.ts — iter-5 per-image (Stage 1) tool schema.
 *
 * Extends the iter-4 COMPARISON_TOOL_SCHEMA with the iter-5 per-image
 * additions specified in W11 (Stage 1 per-image output additions):
 *   - listing_copy.headline + paragraphs (voice-tier-aware)
 *   - appeal_signals + concern_signals + buyer_persona_hints
 *   - retouch_priority + retouch_estimate_minutes
 *   - gallery_position_hint
 *   - social_first_friendly + requires_human_review
 *   - confidence_per_field { room_type, scoring, classification }
 ...
 * inline instruction. We match the iter-4 verbosity standard — specific
 * targets, examples, forbidden patterns.
 *
 * Gemini responseSchema honoured constraints (no $ref, no minLength/maxLength,
 * minItems is fine).
 */

export const COMPARISON_TOOL_NAME_ITER5 = 'classify_image_iter5';

export const COMPARISON_TOOL_SCHEMA_ITER5: Record<string, unknown> = {
  type: 'object',
  properties: {
    // ─── iter-4 fields preserved verbatim (description-as-prompt) ─────────
    analysis: {
      type: 'string',
      description:
