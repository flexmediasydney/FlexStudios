/**
 * iter5VoiceAnchor.ts — voice tier rubric blocks for iter-5 harness.
 *
 * W11.7.7 + W11.7.8 voice anchors. Each rubric block is a 50-100-line
 * prompt-text constant that gets injected into BOTH:
 ...
 *
 * The point: same architectural specificity, different prose register.
 * Apply Belle Property luxury voice to a $1M Punchbowl cottage and the copy
 * reads pretentious. Apply approachable voice to a $7M Mosman pavilion and
 * the copy underqualifies. Tier modulation prevents both.
 *
 * Three presets:
 *   premium      — Belle Property / luxury magazine ($5M+ harbour, Federation, architect)
 ...
 *
 * Override path: when property_voice_anchor_override is set, the entire
 * tier rubric is replaced with the operator's free-text directive
 * (forbidden patterns from standard tier still apply).
 */

export const VOICE_ANCHOR_BLOCK_VERSION = 'iter5-v1.0';
...
/**
 * Sydney typology primer — light version for iter-5 (W11.7.9 lite).
 ...
 *   3. (Sydney primer is separate, injected only in Stage 1 system prompt)
 */
