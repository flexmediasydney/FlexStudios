/**
 * pass1Prompt.snapshot.test.ts — Wave 7 P1-10 (W7.6) byte-identical regression
 * gate. The pre-refactor monolithic builder produced the snapshot in
 * `visionPrompts/__snapshots__/pass1Prompt.snap.txt`; this test replays the
...
import { FIXTURE_ANCHORS } from './visionPrompts/__snapshots__/_fixtures.ts';

Deno.test('pass1Prompt: byte-identical to checked-in snapshot (W7.6 regression gate)', async () => {
  const built = buildPass1Prompt(FIXTURE_ANCHORS);
  const got = `${built.system}\n---\n${built.userPrefix}`;

  const snapPath = new URL(
    './visionPrompts/__snapshots__/pass1Prompt.snap.txt',
    import.meta.url,
  );
  const want = await Deno.readTextFile(snapPath);

  if (got !== want) {
    // Surface a precise diff hint to make the failure debuggable.
