-- ═══════════════════════════════════════════════════════════════════════════
-- Migration 350: Fix pulse_add_agent_to_crm / pulse_add_agency_to_crm
--                column mismatch on agencies INSERT
-- ═══════════════════════════════════════════════════════════════════════════
-- Migration 349 shipped two "Add to CRM" RPCs with two latent bugs that
-- both blocked every promotion attempt:
--
--   1. agencies INSERT referenced columns that don't exist in this schema:
--        - `website`  →  agencies has no website column (canonical source
--                        is pulse_agencies.website + entity_field_sources;
--                        see 178_entity_field_sources_schema.sql)
--        - `source`   →  agents.source exists, agencies.source does NOT
--      Hit immediately on the agency branch, raised
--      `42703: column "website" of relation "agencies" does not exist`.
--
--   2. agents INSERT used relationship_state default `'prospect'`, but the
--      `agents_relationship_state_check` constraint only accepts
--      `'Prospecting' | 'Active' | 'Dormant' | 'Do Not Contact'`. The
--      callsite in PulseAgentIntel.jsx passes the same lowercase string,
--      so even an existing-agency promotion would fail with
--      `23514: violates check constraint "agents_relationship_state_check"`.
--
-- Effect: clicking "Confirm & Add" in the Industry Pulse → Add to CRM
-- dialog raised one of the two errors above, the surrounding transaction
-- aborted, the React mutation showed a generic toast, and the dialog
-- stayed mounted. The user perceived the Save button as doing nothing.
--
-- Fix:
--   - agencies INSERT uses only real columns (name/phone/email/rea_agency_id
--     for the agent RPC; + relationship_state for the agency RPC).
--   - agents INSERT defaults relationship_state to 'Prospecting' (matches
--     the check constraint and the existing AgentForm value).
-- Behaviour is otherwise identical to migration 349.

-- ───────────────────────────────────────────────────────────────────────────
-- 1. pulse_add_agent_to_crm
-- ───────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  ELSIF p_new_agency_payload IS NOT NULL
    ...
    -- agencies has no `website` and no `source` column. Drop both.
    INSERT INTO agencies (
      name, phone, email, rea_agency_id
    ) VALUES (
  ...
  -- Create the CRM agent. agents.source DOES exist, keep it.
  INSERT INTO agents (
    name,
    phone,
    email,
    ...
    -- Reject legacy lowercase 'prospect' (the value the old UI sent before
     -- this migration's frontend fix) and default to the canonical
     -- 'Prospecting' that satisfies agents_relationship_state_check.
     COALESCE(NULLIF(p_agent_payload->>'relationship_state', 'prospect'), 'Prospecting'),
  ...
  -- Flip is_in_crm + write linked_agent_id back-pointer.
  UPDATE pulse_agents
     SET is_in_crm       = true,
...
COMMENT ON FUNCTION pulse_add_agent_to_crm IS
  'Atomically promote a pulse_agents row into the CRM: create/find Agency, create CRM Agent, insert pulse_crm_mappings row, flip is_in_crm + linked_agent_id. Single transaction. Migration 350 fix: agencies INSERT no longer references non-existent website/source columns.';

-- ───────────────────────────────────────────────────────────────────────────
-- 2. pulse_add_agency_to_crm
-- ───────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- agencies has no `website` and no `source` column. Drop both.
  INSERT INTO agencies (
    name, phone, email, rea_agency_id, relationship_state
  ) VALUES (
...
COMMENT ON FUNCTION pulse_add_agency_to_crm IS
  'Atomically promote a pulse_agencies row into the CRM: create CRM Agency, insert pulse_crm_mappings row, flip is_in_crm + linked_agency_id. Single transaction. Migration 350 fix: agencies INSERT no longer references non-existent website/source columns.';
