-- 171_pulse_retention_crm_strict_dual_dim.sql
-- Rewrites the Retention scope RPCs to match the strict-scope dual-dimension
-- model:
--   • Scope = ONLY CRM-mapped entities (pulse_agents.is_in_crm = true and
--     pulse_agencies.is_in_crm = true). Nothing else appears in retention
--     views — this tab is specifically about customer accountability, not
--     market exploration.
--   • Two separate dimensions tracked in parallel:
--       Current = for_sale listings, first_seen_at in window
--       Sold    = sold listings, sold_date in window
--     Each dimension has its own captured / missed / retention % / missed $.
--   • Captured = a project exists at the same property_key with status
--     delivered / scheduled / ready_for_partial / to_be_scheduled.
--
-- The v1 RPCs (pulse_get_retention_agent_scope / _agency_scope from
-- migration 165) are kept for back-compat but the frontend swaps to these
-- v2 names. Old RPCs can be dropped in a later cleanup once no consumers
-- call them.
...
RETURNS TABLE (
  ...
  email text,
  mobile text,
  -- Current-listings dimension (for_sale, first_seen_at in window)
...
AS $$
  WITH crm_agents AS (
    -- Scope is LITERALLY pulse_agents.is_in_crm = true. No other agents appear.
  ...
  -- Current listings (for_sale only, first_seen_at in window)
  curr AS (
  ...
  -- Sold listings (sold_date in window)
  sold AS (
    SELECT l.agent_rea_id,
           count(*) AS total,
           count(*) FILTER (
           ...
           -- Missed value approximation: count × avg sold-price-tier Gold std
           -- (we don't have a direct sold-price missed-quote substrate yet).
           -- Placeholder: 0. Future: introduce a sold-quote substrate that
           -- aggregates what we would have charged at shoot-time.
